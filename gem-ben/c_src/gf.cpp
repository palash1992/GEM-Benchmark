...
void learn_embedding(std::string if_name, std::string of_name,
                     bool verbose, bool is_weighted, int d, float eta,
                     float regu, int max_iter, int print_step)
{
    clock_t t;
    srand(time(NULL));
    t = clock();
...
int main(int argc, char* argv[])
{
    ...
    bool verbose, is_weighted;
    int d, max_iter, print_step;
    float eta, regu;
    if_name = std::string(argv[1]);
    of_name = std::string(argv[2]);
    verbose = char(atoi((argv[3])));
    is_weighted = char(atoi((argv[4])));
    d = atoi(argv[5]);
    eta = atof(argv[6]);
    regu = atof(argv[7]);
    max_iter = atoi(argv[8]);
    print_step = atoi(argv[9]);
    learn_embedding(if_name, of_name, verbose, is_weighted,
                    d, eta, regu, max_iter, print_step);
    return 0;
}
