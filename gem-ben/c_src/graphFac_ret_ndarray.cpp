...
np::ndarray learn_embedding(std::string f_name, bool is_weighted, int d, float eta, float regu, int max_iter)
{
    clock_t t;
    srand(time(NULL));
    t = clock();
    load_network(f_name, is_weighted);
    ...
    for(int idx=0; idx < n_nodes; idx++)
        for(int idx2=0; idx2 < d; idx2++)
            Y[idx][idx2] = X[idx][idx2];

    for(int d_dx=0; d_dx < d; d_dx++)
        free(X[d_dx]);
    free(X);

    
    np::dtype dt1 = np::dtype::get_builtin<float>();
    np::ndarray Y_num = np::from_data(Y, dt1,
                                        p::make_tuple(n_nodes, d),
                                        p::make_tuple(d*sizeof(float), sizeof(float)),
                                        p::object());
    t = clock() - t;
    std::cout << "\tTime taken to convert array to np array:" << ((float)t)/CLOCKS_PER_SEC << " secs" << std::endl;
    return Y_num.copy();
}
